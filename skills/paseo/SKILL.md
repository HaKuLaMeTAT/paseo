...
For document reads use `read_context_file` with `section` set to the exact heading title, or `outline=true` to discover titles without loading the body. A section includes its child headings; line pagination is relative to that section. For JSON evidence use fields/ranges. Return the text content once. At 48000 characters the tool requests a specific `budgetReason` for each missing mandatory section; justified reads remain capped at 8000 characters per call. This review threshold is not the model context window. Use `nextRead` unchanged for truncated pages, adding only the reason. Do not restart the task or open a new session merely to reset it. Do not batch outputs beyond the outer tool limit or bypass the tool with shell. Preserve formal cross-review and evidence freshness rules.
...
For tool discovery, print matching tool names only, not all descriptions or schemas. To display an MCP result, print each text block’s `text` directly; `text(result.content)` serializes the blocks again. Reuse rules already provided in startup context; do not force-read them for confirmation.
