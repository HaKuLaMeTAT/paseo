...
基于 [getpaseo/paseo](https://github.com/getpaseo/paseo)，保留本地 AI agent、多主机连接与加密 Relay，关闭不使用的功能并减少空闲后台工作。当前定制桌面版本为 **0.8.0-lite.5**。
...
- **控制 Codex 上下文增长**：单个工具结果历史预算默认 4000 token，提示按需、分段读取；保持原有角色和交叉复核要求。未改模型窗口或压缩阈值，实际收益不做比例承诺，详见[排查记录](docs/codex-context-audit.md)。
...
当前交付文件为 `Paseo-Lite-0.8.0-lite.5-x64.zip`，约 180 MiB。**完整解压后运行 `Paseo.exe`**，不能只拷贝一个 EXE。添加主机时粘贴目标 daemon 生成的完整 Relay 配对链接。
