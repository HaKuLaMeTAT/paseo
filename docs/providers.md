...
Providers may implement `AgentClient.renameNativeSession(handle, title)`. The manager calls it before committing a title change, for both live agents and stored agents without an active session. Failures propagate to the rename request so a provider-side failure does not appear as a successful synchronized rename. Codex implements this through `thread/name/set` on a short-lived app-server connection, using the native handle or persistence session ID; it does not start a model turn. Providers without this method continue to store titles only in Paseo. This is outbound title synchronization, not a background watcher for names edited in other applications.
