...
async function createBaseRecoveryFixture(baseBranch: string | null) {
  ...
  return { workspace, registry, service, paseoHome, archiveAndRemove };
}

test("archive upgrades a legacy saved name from surviving exact metadata before deletion", async () => {
...
test("restores an already archived legacy base name without inventing a remote identity", async () => {
  const fixture = await createBaseRecoveryFixture("main");
